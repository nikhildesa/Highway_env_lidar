...
"""
      ...
      *******************   lidar -> fc1-> fc2-> output+vx,vy ->fc3 ->fc4 ->output  **********************
...
class DeepQNetwork(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, 
        ...
        self.fc3 = nn.Linear(514,self.fc3_dims)
...
class Agent():
    def __init__(self,max_mem_size,gamma, epsilon, lr, input_dims, batch_size, n_actions,eps_end=0.05, eps_dec=5e-5):
        ...
        self.lidar_memory = np.zeros((self.mem_size, 180), dtype=np.float32)
        self.new_lidar_memory = np.zeros((self.mem_size, 180), dtype=np.float32)
        self.action_memory = np.zeros(self.mem_size, dtype=np.int32)
        self.reward_memory = np.zeros(self.mem_size, dtype=np.float32)
        self.terminal_memory = np.zeros(self.mem_size, dtype=np.bool)
        
        self.velocity_memory = np.zeros((self.mem_size,2), dtype=np.float32)
        self.new_velocity_memory = np.zeros((self.mem_size,2), dtype=np.float32)
        self.current_loss = []    
...
if __name__ == '__main__':
    ...
    agent = Agent(max_mem_size=50000,gamma=0.99, epsilon=1.0, lr=0.003,input_dims=[180], batch_size=32, n_actions=5,eps_end=0.05)
    n_games = 4000
    ...
    with open('Data.csv','w') as out_file:
        for i in range(n_games):
            ...
            while not done:
                ...
                """ Check if rollouts """
                if global_step%500 == 0:
                    rollouts(lidar,velocity,global_step)
                
                """ Take action  """
                action = agent.choose_action(lidar,velocity)
                observation_, reward, done, info = env.step(action)   
                ...
                env.render()                        
                score+=reward
                agent.store_transition(lidar,action, reward,lidar_, done,velocity,velocity_) 
                agent.learn(i)
