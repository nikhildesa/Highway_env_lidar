...
class Agent():
    ...
    def learn(self,global_step):
        ...
        terminal_batch = T.FloatTensor(self.terminal_memory[batch]).to(self.Q_eval.device)
        ...
        # resize tensors
        actions = action_batch.view(action_batch.size(0), 1)   #
        reward_batch = reward_batch.view(reward_batch.size(0), 1)
        dones = terminal_batch.view(terminal_batch.size(0), 1)    #

        # compute loss      
        curr_Q = self.Q_eval.forward(lidar_batch,velocity_batch).gather(1, actions)
        next_Q = self.Q_next.forward(new_lidar_batch,new_velocity_batch)
        
        max_next_Q = T.max(next_Q, 1)[0]
        max_next_Q = max_next_Q.view(max_next_Q.size(0), 1)
        expected_Q = reward_batch + (1 - dones) * self.gamma * max_next_Q
...
if __name__ == '__main__':
    ...
    agent = Agent(max_mem_size=50000,gamma=0.99, epsilon=1.0, lr=1e-4,input_dims=[540], batch_size=32, n_actions=5,eps_end=0.05)
    n_games = 4000
