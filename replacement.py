...
def lidar_visualization(smallest_intersection,sensing_radius,agent_angle):
    center = (0,0)
    for i in range(len(smallest_intersection)):
        x = smallest_intersection[i] * math.cos(math.radians(agent_angle))
        y = smallest_intersection[i] * math.sin(math.radians(agent_angle))
        plt.plot(center,(x,y))
        plt.show()
...
def display_lidar(position):
    ...
    """<------------------------- ray end points------------------------------->"""
    endpoints = find_endpoints(agent,agent_angle,sensing_radius)
    smallest_intersection = []
    for endpoint in endpoints:
        intersection = [sensing_radius]
        for disk_set in neighbor:
            for disk in disk_set:
                flag,point = get_intersection(disk,1,agent,endpoint)
                if flag!=False:
                    distance = np.linalg.norm(agent - point)
                    intersection.append(distance)
        smallest_intersection.append(min(intersection))
    
    """<--------------------------- visualize the lidar--------------------------->"""    
    #lidar_visualization(smallest_intersection,sensing_radius,agent_angle)        
